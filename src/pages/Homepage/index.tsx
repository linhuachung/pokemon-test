...
import { ContentButton, ContentItem, HomePageWrapperContentStyled } from '@/pages/Homepage/style'
...
const Homepage = () => {
   ...
   const { Type: typeList } = useSelector((state: StatePokemon) => state.pokemon)
   ...
   return (
      <Layout>
         <HomePageWrapperContentStyled>
            <ContentButton>
               <ContentItem>
                  {typeList.data.map(type => (
                     <Button type='primary' key={type.name}>{type.name}</Button>
                  ))}
            ...
            <ContentButton>
               <Button type='default' onClick={() => showMoreItem('prev')} disabled={page === 1}> Prev</Button>
               <Button type='primary' onClick={() => showMoreItem('next')}
